...
chunks are already silence-gated during offline capture replay.
"""
...
def run_single_case(
    *,
    ...
    try:
        ...
        for wav_path, _rms, start_sample, end_sample in iter_chunks_from_wav(
            audio_path, leveler_settings=leveler_settings, temp_dir=temp_dir
        ):
            chunk_count += 1
            p = Path(wav_path)
...
def summarize_results(results: list[EvalCaseResult]) -> dict:
    """Aggregate mean WER/CER over cases that succeeded."""
    ok = [r for r in results if r.error is None and r.chunk_count > 0]
    if not ok:
        return {"cases": len(results), "ok": 0, "mean_wer": None, "mean_cer": None}
    return {
        "cases": len(results),
        "ok": len(ok),
        "mean_wer": sum(r.wer for r in ok) / len(ok),
        "mean_cer": sum(r.cer for r in ok) / len(ok),
    }
