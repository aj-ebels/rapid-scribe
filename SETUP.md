...
Install the transcription model from the **Models** tab in the app (Download & install); or it will download automatically the first time you click **Start**. For NVIDIA GPU, use `onnx-asr[gpu,hub]` in `requirements.txt` instead of `onnx-asr[cpu,hub]` (requires CUDA).
...
4. Open the **Models** tab and click **Download & install** for the recommended model (or it will download the first time you click **Start**).
...
- **No model yet:** Use the **Models** tab → **Download & install** to install the recommended transcription model before (or after) your first recording.
- **Download interrupted:** If the model download was interrupted, open the **Models** tab and click **Download & install** again; Hugging Face usually resumes the download.
